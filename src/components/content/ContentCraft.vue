...
async function saveContent() {
  ...
  if (props.content) {
    const { data, error } = await supabase
      .from('content')
      .update(payload)
      .eq('id', props.content.id)
      .select()
      .single()

    if (error) return console.error('❌ Failed to update content:', error)
    ...
    if (error) return console.error('❌ Failed to insert content:', error)
...
function getIframeContent() {
  const iframeEl = props.iframeRef?.value
  ...
  const rawText = iframeEl.contentDocument.body.innerText.trim()
...
<template>
  <div v-if="!showList">
    <TitleCraft
      ...
      :button-secondary="{ text: 'Quit', type: 'quit', action: quitContent }"
    />

    <button @click="getIframeContent">Get content</button>
